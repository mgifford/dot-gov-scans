...
- Steps:
  1. Implement `POST /v1/jurisdictions/{jurisdictionCode}/scans` in `src/api/scans.py`.
  2. Accept optional `runMonth` and `includeFederal` payload fields.
...
- Purpose: Provide latest and month-specific jurisdiction artifact retrieval.
- Steps:
  1. Implement `GET /v1/jurisdictions/{jurisdictionCode}/toon/latest` in `src/api/artifacts.py`.
  2. Implement `GET /v1/jurisdictions/{jurisdictionCode}/toon/{runMonth}` in same module.
...
- Steps:
  1. Implement `GET /v1/jurisdictions/{jurisdictionCode}/hosts/{hostname}` in `src/api/hosts.py`.
