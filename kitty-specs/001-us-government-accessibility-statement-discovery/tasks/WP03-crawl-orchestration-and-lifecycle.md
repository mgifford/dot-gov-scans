...
- Success criteria:
  - Per-jurisdiction run lock prevents concurrent active scans.
...
- Purpose: Execute jurisdiction scan jobs at monthly cadence with guarded concurrency.
- Steps:
  1. Implement `src/jobs/monthly_scan_job.py`.
  2. Enforce one-active-run-per-jurisdiction lock semantics.
