# Work Packages: US Government Accessibility Statement Discovery

**Inputs**: Design documents from `/kitty-specs/001-us-government-accessibility-statement-discovery/`  
...
### Included Subtasks
- [ ] T007 Implement translation glossary model and loader for the configured glossary language set in `src/glossary/glossary_store.py`
...
### Implementation Notes
- Glossary completeness is required for the configured glossary language set before this package is considered done.
...
- [ ] T017 Implement monthly scheduler job orchestration with one-active-run-per-jurisdiction guard in `src/jobs/monthly_scan_job.py`
...
### Risks & Mitigations
- Risk: scheduler overlap per jurisdiction. Mitigation: lock by jurisdiction and reject concurrent run with deterministic status.
...
**Independent Test**: Completed scan writes jurisdiction TOON artifact containing required domain fields; latest and monthly retrieval resolve deterministically.
...
- [ ] T022 Implement latest-snapshot resolver for each jurisdiction in `src/storage/latest_resolver.py`
- [ ] T023 [P] Implement CLI utility to generate/replay jurisdiction artifacts in `src/cli/generate_country_artifact.py`
...
### Included Subtasks
- [ ] T025 Implement `POST /v1/jurisdictions/{jurisdictionCode}/scans` with idempotent monthly trigger semantics in `src/api/scans.py`
- [ ] T026 [P] Implement `GET /v1/scans/{scanId}` status endpoint in `src/api/scans.py`
- [ ] T027 [P] Implement jurisdiction artifact retrieval endpoints (`latest`, `runMonth`) in `src/api/artifacts.py`
...
- [ ] T035 Update operator and developer docs in `README.md` and `kitty-specs/001-us-government-accessibility-statement-discovery/quickstart.md`
...
- Preserve monthly cadence defaults and US-first scope in docs and checks.
...
- **MVP Scope**: WP01 + WP02 + WP03 + WP04 (first end-to-end jurisdiction scan and TOON artifact).
...
| T007 | Implement project glossary store | WP02 | P1 | No |
