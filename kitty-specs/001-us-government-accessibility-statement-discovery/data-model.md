# Data Model - US Government Accessibility Statement Discovery
...
Validation rules:
- One active `CountryScan` per `jurisdiction_code` at a time.
- One completed monthly artifact per `jurisdiction_code` + `run_month`.
...
Validation rules:
- Canonical uniqueness key is `jurisdiction_code + canonical_hostname`.
...
Validation rules:
- Must include entries for the configured glossary language set.
...
- country metadata (`jurisdiction_code`, `run_month`, `scan_id`)
