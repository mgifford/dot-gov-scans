# Feature Specification: US Government Accessibility Statement Discovery

**Feature Branch**: `001-us-government-accessibility-statement-discovery`  
**Created**: 2026-02-26  
**Status**: Draft  
**Input**: User description: "I want to find accessibility statements in government websites where they are required. This is focused on the United States, organized by states and federal scope."
...
As a government compliance officer, I can review state and federal level government domains that are in scope of the US public-sector accessibility requirements and see whether an accessibility statement was found for each domain.

**Why this priority**: This is the core compliance outcome and delivers immediate value for legal and policy monitoring.

**Independent Test**: Can be fully tested by running a jurisdiction scan on an input domain list and verifying the output includes statement presence status and statement URL (when found) for each in-scope domain.
...
**Why this priority**: Reliable domain identity is necessary for trustworthy compliance reporting and jurisdiction comparisons.
...
As a compliance officer or accessibility advocate, I can use state/federal-specific cached TOON outputs that include key findings and sample URLs for downstream audits and accessibility spot-checks.

**Why this priority**: Structured jurisdiction outputs support repeatable monitoring, sharing, and follow-up analysis.
...
1. **Given** a completed jurisdiction scan, **When** output is saved, **Then** a state/federal-specific TOON file is created and available for cached reuse.
...
- **FR-001**: The system MUST ingest state-organized and federal government domain lists as input for scanning, including lists published by national authorities and curated public-sector repositories.
- **FR-002**: The system MUST support scanning with US state and federal scope as the default focus.
- **FR-003**: The system MUST classify each scanned domain as in-scope for required accessibility statements based on US public-sector accessibility requirements applicability rules for public-sector entities.
- **FR-004**: The system MUST detect accessibility statement evidence using multilingual term matching suitable for US-government-relevant language variants in the configured glossary.
...
- **FR-011**: The system MUST produce one cached TOON output file per jurisdiction scan.
- **FR-012**: The system MUST include, at minimum, canonical domain, statement URL (if any), detection confidence, detected language (if any), redirect chain, and sampled URLs in each jurisdiction output.
...
- **FR-017**: The system MUST maintain and use a translation glossary for accessibility-related detection terms that covers the configured glossary language set.

### Key Entities *(include if feature involves data)*

- **Jurisdiction Scan**: A run scope representing one country, its domain set, timestamp, and cached output artifact.
...
- **Translation Glossary**: A managed set of accessibility-related terms and equivalents across the configured glossary language set used for multilingual detection.
...
- **SC-001**: For a completed jurisdiction scan, 100% of input domains receive a recorded detection status (found, not found, or uncertain).
...
- **SC-005**: Compliance officers can identify statement presence/absence for any scanned country dataset in under 5 minutes using only the jurisdiction TOON output.

## Assumptions

- The first release focuses on public-sector entities covered by the US public-sector accessibility requirements; broader legal scopes are out of scope.
- Government-domain confidence scoring beyond source provenance is deferred to a later phase.
- Federal support is enabled in scope handling but United States remains the primary operational focus for initial rollout.
